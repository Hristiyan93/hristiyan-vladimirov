...
function writeCSV($items){
    $headers = array();
    $fp = fopen("brake_wear_sensors-specifics1.csv", "a");
    ...
    array_unshift($headers, "EbayId");
    fputcsv($fp, $headers);

    foreach ($items as $id => $item) {
        $toWrite = array();
        $toWrite = array_fill_keys($headers, '');
        $toWrite['EbayId'] = $id;
...
// foreach ($files as $key => $fileName) {
    ...
    $fp = fopen("scraped_info2.csv", "w");
    // fputcsv($fp, array('Reference', 'RefNumber', 'GTpart', 'EbayId', 'MPN', 'StoreName', 'Price', 'Title', 'Brand', 'Sold'));
    fputcsv($fp, array('EbayId', 'MPN', 'Price', 'Title', 'Brand', 'Sold'));
    $itemSpecifics = array();
    $dom = new simple_html_dom();

    foreach ($input as $n => $oem) {
        // $gtpart = $row['G-Part'];
        // if(!empty($row['Klarius PartNo'])){
        //     $ref = 'Klarius';
        //     $oem = $row['Klarius PartNo'];
        // }
        // elseif(!empty($row['BOSAL PartNo'])){
        //     $oem = $row['BOSAL PartNo'];
        //     $ref = 'Bosal';
        // }
        // else
        //     continue;
        ...
        foreach ($results as $id => $res) {
            ...
            if(!empty($specs)){
                ...
                $title = $html->find("#itemTitle", 0);
                $item['title'] = htmlspecialchars_decode(trim(str_replace("Details about  &nbsp;", "", $title->plaintext)));
                ...
                $sold = $html->find(".soldwithfeedback", 0);
                if($sold != null){
