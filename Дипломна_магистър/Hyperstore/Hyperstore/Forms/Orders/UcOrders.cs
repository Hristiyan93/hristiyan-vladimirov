    ...
    public partial class UcOrders : UserControl
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (order != null && order.statusId == 1) { 
                using (FrmCompleteOrder frm = new FrmCompleteOrder(order))
                {
                    frm.ShowDialog();
                    if (frm.DialogResult == DialogResult.OK)
                    {
                        ...
                        {
                            Stock stock = db.Stocks.FirstOrDefault(s => s.productId == product.productId && s.departmentId == order.departmentId);
                            if (stock == null)
                            {
                                stock = db.Stocks.Create();
                                stock.departmentId = order.departmentId;
                                stock.productId = product.productId;
                                db.Stocks.Add(stock);
                            }
                            stock.qty = product.qtyReceived;
                        }
            ...
            {
                MessageBox.Show("Тази поръчка е вече завършена и не може да се приема.", "Приемане поръчка", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }
        }
    }
}
